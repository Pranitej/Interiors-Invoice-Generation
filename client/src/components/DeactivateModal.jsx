...
export default function DeactivateModal({ companyId, companyName, onSave, onClose }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError("");
    if (!remarks.trim()) { setError("Remarks are required to deactivate a company"); return; }
    setSubmitting(true);
    try {
      ...
      <div className="bg-white dark:bg-gray-800 w-full max-w-md rounded-2xl border border-gray-200 dark:border-gray-700 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <p className="text-sm text-gray-600 dark:text-gray-300">
            This will immediately deactivate the company. Users will not be able to create or edit invoices.
          </p>
