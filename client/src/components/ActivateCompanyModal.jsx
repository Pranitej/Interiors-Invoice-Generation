// client/src/components/ActivateCompanyModal.jsx
import { useState } from "react";
import { X, CheckCircle } from "lucide-react";
import API from "../api/api";

export default function ActivateCompanyModal({ companyId, companyName, onSave, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const res = await API.patch(`/subscription/companies/${companyId}/activate-company`, { remarks });
      ...
      <div className="bg-white dark:bg-gray-800 w-full max-w-md rounded-2xl border border-gray-200 dark:border-gray-700 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <p className="text-sm text-gray-600 dark:text-gray-300">
            This will manually reactivate the company. The subscription and feature toggles are not affected.
          </p>
          ...
          <div>
            ...
            <textarea
              rows={3}
              placeholder="e.g. Reactivated after manual review — subscription renewal in progress"
              value={remarks}
          ...
          <div className="flex gap-2.5 pt-1">
            ...
            >
              {submitting ? "Activating..." : "Activate Company"}
            </button>
            <button
              type="button"
              onClick={onClose}
